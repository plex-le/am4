...
#[inline]
pub fn fuel(ac_fuel: f32, dist: Distance, training: FuelTraining, ci: Ci) -> f32 {
    let training_factor = 1.0 - training.get() as f32 / 100.0;
    let dist_val = (dist.get() * 100.0).ceil() / 100.0;
...
#[inline]
pub fn contribution(dist: Distance, game_mode: GameMode, ci: Ci) -> f32 {
    let d = dist.get();
    let k = if d > 10000.0 {
        0.0048
    } else if d > 6000.0 {
...
/// Calculate income based on the configuration, ticket price, and user training/load settings.
#[inline]
pub fn income(
    cfg: &ConfigVariant,
    ticket: &Ticket,
    ...
    match (cfg, ticket) {
        (ConfigVariant::Pax(c), Ticket::Pax(t)) => {
        ...
        (ConfigVariant::Pax(c), Ticket::VIP(t)) => {
            let y_inc = c.y as f32 * t.y as f32;
            let j_inc = c.j as f32 * t.j as f32;
            let f_inc = c.f as f32 * t.f as f32;
            (y_inc + j_inc + f_inc) * load_factor
